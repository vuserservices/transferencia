...
class LTDA:
    ...
    def ejecutar(self):
        ...
        try:
            ...
            Search_PJ = re.search(r'DE (SOCIEDAD POR ACCIONES|EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA|SOCIEDAD DE RESPONSABILIDAD LIMITADA) (.+?)(?: SpA| EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA| SOCIEDAD DE RESPONSABILIDAD LIMITADA| LIMITADA)(.+?)(?: SpA| EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA| SOCIEDAD DE RESPONSABILIDAD LIMITADA| LIMITADA)', content, re.DOTALL)
        ...
        try:
            ...
            if razon_social is not None and ultimoestadoempresa=="TRANSFORMACIÓN":
                razon_social = razon_social.group(3)+' '+tipoempresa
        ...
        try:
            ...
            texto_accionistasrepresentantesSIIv1match= re.search(r'ARTÍCULO QUINTO DEL CAPITAL SOCIAL:(.*?)ARTÍCULO SEXTO DE LA RESPONSABILIDAD (DE LOS SOCIOS|DEL SOCIO):', content, re.DOTALL)
            ...
            if texto_accionistasrepresentantesSIIv1match:
                ...
                for  letra, cuenta in cuentaacciionistas:
                    ...
                    searchrut= searchrut.group(2) or searchrut.group(3)

                    accionrut=re.search(r'(equivalente al (.*?) por| de un (.*?) por)', cuenta, re.DOTALL)
                    accionrut= accionrut.group(2) or accionrut.group(3)
                    
                  
                    montorut=re.search(r'aporte de \$(.*?)pesos', cuenta, re.DOTALL)
                    

                    administradoresdicc={}
                    administradoresdicc["tipo"]="administrador"
                    administradoresdicc["provisorio"]="false"  
                    nombrerutencontrado = re.search(re.escape(searchrut.strip()) + r', Rut ([0-9Kk.-]+),', content)
                    administradoresdicc["rut"]=nombrerutencontrado.group(1)
                  
                    


                    administradoresdicc["nombre"]=searchrut
                    administradoresdicc["comoAdministra"]="separadamente"

                    nombrecompleto=searchrut.strip().split(" ")
        ...
        try:
            ...
            if articulo5match:
                ...
                articulo5monto=re.search(r'\$(.*?)\s',articulo5match.group(1),re.DOTALL).group(1)
            ...
            if texto_accionistasrepresentantesSIIv1match:
                ...
                firmantesdetalle= re.findall(r'([A-Z])\)\s(.*?)se obliga a enterar el equivalente al (.*?) por ciento del capital social, mediante el aporte de \$(.*?)\s', texto_accionistasrepresentantesSIIv1match.group(1),re.DOTALL)
                ...
                for  letra, cuenta in cuentaacciionistas:
                    ...
                    montorut = re.search(r'aporte de \$(.*?)pesos|cantidad de \$(.*?) pesos', cuenta, re.DOTALL)
                    montorut = montorut.group(2) or montorut.group(3)

                    firmantesdetalledicc["nombre"] = searchrut
                    firmantesdetalledicc["participacion"] = int(accionrut.replace(".", ""))
                    firmantesdetalledicc["participacionDinero"] = montorut.replace(".", "")
                    firmantesdetalledicc["frasereferencia"] = searchrut + " se obliga a enterar el equivalente al " + accionrut + " por ciento del capital social, mediante el aporte de $" + montorut + " pesos"
                    nombrerutencontrado = re.search(re.escape(searchrut.strip()) + r', Rut ([0-9Kk.-]+),', content)
            ...
            if fraseaccionistasreferenciatrue=="false" or errorconnombres=="false" or  articulo5true=="false" or articuloseptimotrue=="false":
        ...
        try:
            ...
            if textoinicioaarticulo1:           
                ...
                domiciliofinal=re.search(r'El domicilio de la (Sociedad|Empresa) es(.*?),\ssin perjuicio',textoinicioaarticulo1.group(1),re.DOTALL).group(1)
        ...
        #region NOMBRE DE FANTASIA y objeto
            ...
            if articulosegundocompleto:
                ...
                articulosegundoobjeto= re.search(r'La (Sociedad|Empresa) tendrá por objeto desarrollar las siguientes actividades:(.*?)\.', articulosegundocompleto.group(1), re.DOTALL ).group(1)
        ...
        if errorextraccion and errorfatal and empresatransformada and empresadisuelta == "false":
                    ...
                    "prestudios": [
                        {
                            ...
                            "administracion": {
                                ...
                                "valor": {
                                    ...
                                    "administradores": administradoreslist
                                }
                            },
                            "capital": {
                                "ok": articulo5true,
