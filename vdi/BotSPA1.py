...
class SPA:
    ...
    def ejecutar(self):
        inicio = datetime.now()
        print("este es el script de SPA........")
        ...
        try:
            ...
            match_LTDA= re.search(r'(SOCIEDAD DE RESPONSABILIDAD LIMITADA|(?:.*?LIMITADA))',Search_PJ.group(), re.DOTALL)
        ...
        try:
            ...
            razon_social = re.search(r'DE (SOCIEDAD POR ACCIONES|EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA|SOCIEDAD DE RESPONSABILIDAD LIMITADA) (.+?) (SpA|EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA|SOCIEDAD DE RESPONSABILIDAD LIMITADA)', content, re.DOTALL)                      
        ...
        try:
            ...
            texto_accionistasrepresentantesSIIv1= re.search(r'El o la representante ante el SII es (.*?)\.\s', content, re.DOTALL)
            texto_accionistasrepresentantesSIIv12omas= re.search(r'Los representantes ante el SII son: (.*?)\.\s', content, re.DOTALL)
            if texto_accionistasrepresentantesSIIv1 is not None and texto_accionistasrepresentantesSIIv12omas is None:
                try:

                    nombreyrutrepresentantebruto= texto_accionistasrepresentantesSIIv1.group(1).replace(" Rut ","")
                    nombreyrutrepresentanteseparado= nombreyrutrepresentantebruto.split(",")
                    nombreseparadoSII=nombreyrutrepresentanteseparado[0].strip()
                    rutseparadoSII=nombreyrutrepresentanteseparado[1].strip()
                ...
                pass
            if texto_accionistasrepresentantesSIIv12omas is not None and texto_accionistasrepresentantesSIIv1 is None:
                try:
                    texto_accionistassplit=texto_accionistasrepresentantesSIIv12omas.group(1).split(';')
            ...
            transitorios = re.search(r'TITULO QUINTO.- OTROS PACTOS(.*?) El presente instrumento ', content, re.DOTALL)
            ...
            if transitorios:
                ...
                matches_letras = re.findall(r'([A-Z])\)\s(.*?)(?=\s[A-Z]\)|\sARTÍCULO|$|DOCUMENTOS REGISTRABLES)', transitorios.group(1), re.DOTALL)
                ...
                for letra, contenidoletras in matches_letras:
                    ...
                    for contenidoletras in nombres1:
                        try:
                            ...
                            rutaccionistasunoauno=rutseparadoSII=re.search(re.escape(contenidoletras.strip())+r'\, Rut (.*?)(\.\s|\;\s)',content, re.DOTALL).group(1).split(' ')[0]
        ...
        try:
            ...
            if articulo_6:
                ...
                matches = re.finditer(r'(?:un|\d+)\s*(Directorio|Director(?:es)?|Administrador(?:es)?|Gerente General(?:es)?|Directorio)', referenciafrase)
                cantidad_administradores = 0
                cantidad_directores = 0
                cantidad_gerentes_generales = 0
                ...
                for match in matches:
                    ...
                    elif 'Gerente' in tipo_administrador:
                ...
                directoriofrase = re.search(r'El directorio estará compuesto por (\d+)\s', articulo_6, re.DOTALL)
                if directoriofrase:
                    cantidad_directores=int(directoriofrase.group(1))
                    sumaadministradores=int(directoriofrase.group(1))
                else: pass
...
################
            if sumaadministradores>1:
                ...
                try:
                    ...
                    frasead = re.search(r'Administrador(?:es)?:(.*?)($|quienes actuarán)', articulo_6, re.DOTALL)
                    ...
                    for nombre_rut in listfrasead:
                        ...
                        if len(nombresplit)==5:
                            ...
                            pass
                        if len(nombresplit)==6:
                            nombre1y2= nombresplit[0]+ ' ' + nombresplit[1]+ ' ' + nombresplit[2]+ ' ' + nombresplit[3]
                            apellido1= nombresplit[4] 
                            apellido2= nombresplit[5] 
                        else:
                            pass
                ...
                try:
                    ...
                    frasegg = re.search(r'(Gerente(?:s) General(?:es)|Gerente General será designado en estos estatutos, cuya individualización es):(.*?)\.\s', articulo_6, re.DOTALL)

                    listfrasegg=frasegg.group(2).split(';')
                ...
                try:
                    if cantidad_directores >0:
                        frasedi= re.search(r'Los representantes ante el SII son: (.*?)\.\s', articulo_6, re.DOTALL)   
                    ...
                    for nombre_rut in listfrasedi:
                        ...
                        # Dividir cada nombre y rut por ','
                        nombredi, rut_part = map(str, nombre_rut.split(','))

                        nombresplit= nombredi.split(' ')
...
############CUADNO ES UNO
            if sumaadministradores==1:
                ...
                if cantidad_gerentes_generales==1:
                    ...
                    try:
                        ...
                        frasegg = re.search(r'(El Gerente General será designado en estos estatutos, cuya individualización es:|General:) (.*?)$', articulo_6, re.DOTALL)

                        if frasegg:
                            textofrasegg = frasegg.group(2)
                ...
                if cantidad_directores>0:
                    try:
                        frasedi = re.search(r'(El Director será designado en estos estatutos, cuya individualización es:|Director:) (.*?)$', articulo_6, re.DOTALL)
                        
                        if frasedi:
                            textofrasedi = frasedi.group(2)
                ...
                if cantidad_administradores>0:
                    ...
                    try:
                        ...
                        frasead = re.search(r'(El Administrador será designado en estos estatutos, cuya individualización es:|Administrador:) (.*?)$', articulo_6, re.DOTALL)
                        if frasead:
                            textofrasead = frasead.group(2)
        ...
        try:
            ...
            articuloquinto= re.search(r'ARTICULO QUINTO: (.*?) TITULO TERCERO:', content, re.DOTALL)
            
            
            if articuloquinto:
                artquintofrase= articuloquinto.group(1)
                artquinto_monto= re.search(r'(\$|CL)(.*?)\s', artquintofrase, re.DOTALL).group(2)
                capitaltrue= "true"
            ...
            if art_domicilio_match:
                ...
                texto_domicilio = re.search(r'El domicilio de la Sociedad será\s(.*?)\, sin', art_domicilio_match.group(1), re.DOTALL | re.IGNORECASE).group(1)
        ...
        try:
            ...
            if (articuloprimeroexiste == "false") or (articulocuartotrue == "false"):
                    ...
                    "prestudios": [
                        {
                            ...
                            "accionistas": {
                                "ok": validador_accionistas,
                                "mensajes": [countaccionistas],
                                "frasesReferencia": texto_accionistas_referencia,
                                "articulosReferencia": [
                                    articulo_referencia
                                ],
